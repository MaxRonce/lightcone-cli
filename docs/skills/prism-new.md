...
`/prism-new` takes a research question (or reads it from context) and produces a complete, well-scoped `astra.yaml` backed by literature evidence. It is the first thing a researcher runs in a new project.
...
- Searches for relevant papers using `WebSearch` and reads them via the `prism-extractor` subagent.
...
- Sub-analyses should be created with `prism init --sub-analysis` rather than inlined.
- Never skip Phase 3 even for "obvious" methodological choices.

## Related

- [prism-build](prism-build.md) — the next step after `/prism-new`
- `claude/prism/guides/astra-reference.md` — full `astra.yaml` spec
