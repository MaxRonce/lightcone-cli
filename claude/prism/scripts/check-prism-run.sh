        ...
        if token.endswith('.py'):
            ...
            if match:
                ...
                if out_status == 'pending':
                    print(f'WARNING: You just ran the script for output \`{out_id}\` (status: pending in {universe}), which has an integrated recipe. Use \`prism run {out_id} --universe {universe}\` instead to ensure reproducibility.')
                elif out_status == 'materialized':
                    print(f'NOTE: Output \`{out_id}\` already has results in {universe} from \`prism run\`. If regenerating, use \`prism run {out_id} --universe {universe}\` to keep results reproducible.')
                sys.exit(0)
            break
    ...
    for i, p in enumerate(parts):
        if p == '-m' and i + 1 < len(parts):
            ...
            if match:
                out_id, out_status = match
                if out_status == 'pending':
                    print(f'WARNING: You just ran the module for output \`{out_id}\` (status: pending in {universe}), which has an integrated recipe. Use \`prism run {out_id} --universe {universe}\` instead.')
                elif out_status == 'materialized':
                    print(f'NOTE: Output \`{out_id}\` already has results in {universe} from \`prism run\`. If regenerating, use \`prism run {out_id} --universe {universe}\`.')
                sys.exit(0)
            break
" "$command" 2>/dev/null)
