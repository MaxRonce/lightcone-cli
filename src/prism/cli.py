...
def _create_claude_md(directory: Path, target: str | None = None) -> None:
    ...
    # Append site-specific guidance if a target was specified
    if target:
        from prism.dagster.sites import detect_site, get_site_guidance

        site_key = detect_site(target)
        if site_key:
            guidance = get_site_guidance(site_key)
            if guidance:
                # Insert before the autogenerated section
